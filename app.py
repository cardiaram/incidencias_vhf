# app.py - Servidor completo para gestão de incidencias VHF
...
CORS(app)
app.secret_key = "clave_secreta_para_sesiones_cambiar_despues_2025"
...
FROM_NAME = "Sistema de Gestão de Incidentes VHF"
...
def ler_config():
    ...
    config_padrao = {
        "users": {
            "admin": {"password": "admin2025", "role": "admin", "name": "Administrador"},
            "lacv": {"password": "lacv2025", "role": "operador", "name": "Operador LACV"},
            "inco": {"password": "inco2025", "role": "tecnico", "name": "Técnico INCO"}
        },
        "emails": {
...
def get_emails_destino(tipo):
    config = ler_config()
    return config.get('emails', {}).get(tipo, [])
...
def enviar_correo(tipo, asunto, corpo_texto, logo_path=None):
    ...
    senha = smtp_cfg.get('password', SMTP_PASSWORD)
...
def gerar_correo_incidencia(inc, tipo='criacao'):
    ...
    # CORREÇÃO: Classificação (ALTA/BAIXA)
    clas_valor = inc.get('clasificacao', '')
    if clas_valor == 'Crítica' or clas_valor == 'Alta' or clas_valor == 'ALTA':
        classificacao = '🔴 ALTA'
    else:
        classificacao = '🟢 BAIXA'
    ...
    remoto_fim = inc.get('remoto', {}).get('fin', '-')
    remoto_duracao = inc.get('remoto', {}).get('duracion', '-')
    remoto_resolvido = '✅ Sim' if inc.get('remoto', {}).get('resuelta') == 'Si' else '❌ Não'
...
┌═══════════════════════════════════════════════════════════════════════════════════════════════════┐
│                                                                                                    │
│   📧 Mensagem automática do sistema de gestão de incidentes VHF                                   │
...
def gerar_correo_solicitacao(tipo, dados):
    ...
    if tipo in ['acesso', 'atualizacao_acesso']:
        asunto = f"🔐 {tipo_texto}SOLICITAÇÃO DE ACESSO - #{inc_id} - {estacao}"
        corpo = f"""
┌══════════════════════════════════════════════════════════════════════════════┐
...
┌══════════════════════════════════════════════════════════════════════════════┐
│              Por favor, gerenciar as permissões de acesso necessárias        │
└══════════════════════════════════════════════════════════════════════════════┘
"""
    else:
        asunto = f"✈️ {tipo_texto}SOLICITAÇÃO DE VIAGEM - #{inc_id} - {estacao}"
        corpo = f"""
┌══════════════════════════════════════════════════════════════════════════════┐
...
┌══════════════════════════════════════════════════════════════════════════════┐
│               Por favor, gerenciar o deslocamento e a logística necessária   │
└══════════════════════════════════════════════════════════════════════════════┘
"""
    return asunto, corpo
    ...
    try:
        ...
        for inc in incidencias:
            if 'remoto' not in inc:
                inc['remoto'] = {}
            if 'presencial' not in inc:
                inc['presencial'] = {}
            if 'acessos' not in inc:
                inc['acessos'] = []
            if 'viagens' not in inc:
                inc['viagens'] = []
...
def api_enviar_correo():
    try:
        ...
        # CORREÇÃO: Classificação para o assunto do email
        clas_valor = incidencia.get('clasificacion', '')
        if clas_valor == 'Crítica' or clas_valor == 'Alta' or clas_valor == 'ALTA':
            classificacao = 'ALTA'
        else:
            classificacao = 'BAIXA'
...
def api_enviar_solicitud():
    try:
        ...
        # Mapear tipo para configuração de email
...
def add_user():
    try:
        ...
        if 'users' not in config:
            config['users'] = {}
        config['users'][username] = {'password': password, 'role': role, 'name': name}
...
def delete_user(username):
    try:
        if username == 'admin':
            return jsonify({'success': False, 'error': 'Não é possível remover o usuário admin'}), 400
        config = ler_config()
...
def update_emails():
    try:
        ...
        if not tipo:
            return jsonify({'success': False, 'error': 'Tipo não especificado'}), 400
        config = ler_config()
...
def update_station(id):
    try:
        ...
        for i, s in enumerate(stations):
            if s.get('id') == id:
                stations[i]['name'] = data.get('name', s['name'])
                stations[i]['type'] = data.get('type', s['type'])
                stations[i]['active'] = data.get('active', s['active'])
                if guardar_config(config):
                    return jsonify({'success': True})
                return jsonify({'success': False}), 500
        return jsonify({'success': False, 'error': 'Estação não encontrada'}), 404
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
        ...
        if station_name:
            for inc in incidencias:
                if inc.get('estacion') == station_name:
                    return jsonify({'success': False, 'error': 'Estação tem incidentes associados'}), 400
...
def save_smtp_config():
    try:
        ...
        global SMTP_SERVER, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, FROM_NAME
        SMTP_SERVER = data.get('server')
        SMTP_PORT = data.get('port')
        SMTP_USER = data.get('username')
...
def test_email_config():
    try:
        ...
        if not destinatario:
            return jsonify({'success': False, 'error': 'Destinatário não informado'}), 400
...
def mostrar_info_red():
    ...
    try:
        ...
        print(f"📍 Acesso REDE: http://{ip_local}:{PORT}")
    except:
        pass
    print(f"\n📁 Dados: {os.path.abspath(DATA_FILE)}")
    print(f"⚙️ Configuração: {os.path.abspath(CONFIG_FILE)}")
    config = ler_config()
    ...
    print("\n✅ Servidor pronto. Ctrl+C para parar.")
    print("="*60 + "\n")
